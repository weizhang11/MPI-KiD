...
module test_cases_2d
  ...
  Use parameters, only : dt, num_h_moments, num_h_bins &
     , aero_N_init, aero_rd_init, aero_sig_init,nx,pnx
...
#ifdef USE_MPI
    if (.not. allocated(save2dfield_dp) ) allocate(save2dfield_dp(nz,1:pnx))
    call update_buffer_nobdy_real_dp (field_mask)
    call comm_master_worker_zx_dp(-1,save2dfield_dp(:,1:pnx),field_mask(:,1:nx))
    if ( rank == master ) then
       allocate(gfield_mask(nz,0:pnx+1))
       gfield_mask(:,:) = 1.
       gfield_mask(:,1:pnx) = save2dfield_dp(:,1:pnx)
    endif
    deallocate(save2dfield_dp)

    if (.not. allocated(save1dfield_dp) ) allocate(save1dfield_dp(1:pnx))
    call comm_master_worker_x_dp(-1,save1dfield_dp(1:pnx),x(1:nx))
    if ( rank == master ) then
       allocate(gx(pnx))
       gx(1:pnx) = save1dfield_dp(1:pnx)
    endif
...
#ifdef USE_MPI
    do j=0, nx+1
       x(j)=maxX*pidx(j)/float(pnx) ! parall
    enddo
    xs = 0. ; xe = maxX * pnx / float(pnx)
#else
    do j=0, nx+1
       x(j)=maxX*j/float(nx)
    enddo
...
#ifdef USE_MPI
    call update_buffer_nobdy_real_dp (theta)
    call update_buffer_nobdy_real_dp (qv)
...
#ifdef USE_MPI
    do j=0, nx+1
       x(j)=maxX*pidx(j-1)/float(pnx)-full_xlevel_offset ! parall
    enddo
#else
    do j=0, nx+1
    ...
    do itime=1,n_times
       ...
       DO I=1,NX+1
          ...
          DO K=1,NZ+1
             ZZ1=zp(k)/ZSCALE1
             ZZ2=zp(k)/ZSCALE2
             XL=2.*SQRT((xptmp-XCEN)**2)
             XX=XL/XSCALE
             ZL=2.*ZSCALE1
             PHI(i,k)=0.
             IF (XX.LE.1.) THEN
                IF(ZZ1.LT.1.) THEN
                   PHI(i,k)=   (-cos(pi*(xptmp-X0)/XSCALE)*sin(pi*zp(k)/ZL))
                   PHI(i,k)=is*(PHI(i,k)*AMPL)
                ELSEIF (ZZ1.GE.1..AND.ZZ2.LE.1.) THEN
                   ZL=2.*(ZSCALE2-ZSCALE1)
                   PHI(i,k)=   (-cos(pi*(xptmp-X0)/XSCALE) &
...
!      call messages('AAA-----')
           call update_buffer_real_dp(v_t(:,:,itime))
           call update_buffer_real_dp(w_t(:,:,itime))
        else
!      call messages('BBB-----')
!      call kid_mpi_barrier
           call update_buffer_nobdy_real_dp(v_t(:,:,itime))
!      call kid_mpi_barrier
!   call messages('test')
!      call messages('111-----')
!      call kid_mpi_barrier
           call update_buffer_nobdy_real_dp(w_t(:,:,itime))
!      call kid_mpi_barrier
...
#ifdef USE_MPI
       if (l_periodic_bound) then
          call update_buffer_real_dp(v_t_half(:,:,itime))
          call update_buffer_real_dp(w_t_half(:,:,itime))
       else
          call update_buffer_nobdy_real_dp(v_t_half(:,:,itime))
          call update_buffer_nobdy_real_dp(w_t_half(:,:,itime))
       endif
#endif
...
#ifdef USE_MPI
    dxp(1:nx+1) = dx(1:nx+1)
    if ( rank == nworker ) then
       dxp(nx+1) = dx(nx)
    endif
#else
    dxp(1:nx) = dx(1:nx)
...
!      do k=1,nz+1
       ...
       enddo
          k=nz+1
          do i=1,nx
             uz(i,k)=(phi(i+1,k)-phi(i,k))/dxp(i)*dt/dzp(k-1)
          enddo
...
#ifdef USE_MPI
    AX = maxval(phi(:,:))
    call mpi_allreduce(AX,phi_tmp,1,mpi_real8,mpi_max,mpi_comm_world,mpi_ierr)
    AX = (A1*2*X1/pi)/phi_tmp
#else
