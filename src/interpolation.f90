...
! Initialize aerosols and their tendencies
  ...
  subroutine interpolate_forcing
    ...
    if (itime>=1.and. itime < loc_ntimes ) then
       ...
       case(1)
#ifdef USE_MPI
       !!!! This part is needed to be parallelized!!!!
         do k=1,nz
           Tforce(k,0:nx+1)= (thinit(k,0:nx+1) - sum(theta(k,0:nx+1))/(pnx+2)) &
              /exner(k,0:nx+1)/Trelax(k)
           qforce(k,0:nx+1)= (qvinit(k,0:nx+1) - sum(qv(k,0:nx+1))/(pnx+2))/Trelax(k)
         end do
#else
         do k=1,nz
...
#ifdef USE_MPI
  integer function interval_x(x,x0)
    ! Find interval in 1D monotonic array x in which value x0 lies
    ! return the lower index of this interval
...
#ifdef USE_MPI
       do k=1,nxnew
          l=interval_x(x(1:nx-1),xnew(k))
          if (l==0) l=l+1    !extrapolate
          if (l>=(pnx)) l=l-1 !extrapolate
