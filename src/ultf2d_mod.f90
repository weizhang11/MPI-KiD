  ...
  subroutine ultf2d_interface(field, v, w, z, zw, rho, rhow, &
    ...
    if (nx == 1 ) then
...
#ifdef USE_MPI
          call update_buffer_real_reverse_sp(field_ultf2d)
#else
       ...
       if (.not. l_periodic_bound) then
          call update_buffer_nobdy_real_reverse_sp(field_ultf2d)
       endif
#endif
    ...
    if (l_periodic_bound) then 
#ifdef USE_MPI
       call update_buffer_real_dp (field_adv)
#else
...
#ifdef USE_MPI
    if (.not. l_periodic_bound) then 
       call update_buffer_nobdy_real_dp (field_adv)
    endif
#endif
...
#ifdef USE_MPI
      call update_adv_right_buffer_sp (ZF,V,tmp3_mpi)
#endif
...
!..SET UP STENCIL                                                                  
     ...
     &         ) )                                                                 
        ...
        ELSE                                                                       
#ifdef USE_MPI
        if ((J-1+JOFSET) == -1 ) then
          SUM_CFL_OUT=DT*( ARDZ(K)*                                &                
     &      (MAX(0.,W(J+JOFSET,K))+ABS(MIN(0.,W(J+JOFSET,K-1))))   &                
     &      +CY*(MAX(0.,V(J+JOFSET,K))                             &                
     &         +ABS(MIN(0.,tmp3_mpi(2,K)))) )
          FYU=tmp3_mpi(1,K)*ADVPOS + ZF(J+2,K)*ADVNEG                          
        else
          SUM_CFL_OUT=DT*( ARDZ(K)*                                &                
     &      (MAX(0.,W(J+JOFSET,K))+ABS(MIN(0.,W(J+JOFSET,K-1))))   &                
     &      +CY*(MAX(0.,V(J+JOFSET,K))                             &                
...
!..SET UP STENCIL                                                                  
        ...
        if ( J == 0 ) then
         FYU = tmp3_mpi(1,K)*ADVPOS + ZF(J+2,K)*ADVNEG                                  
        elseif ( J == JJP ) then
         FYU = ZF(J-1,K)*ADVPOS + tmp3_mpi(3,K)*ADVNEG                                  
        else
...
#ifdef USE_MPI
       call update_right_buffer_real_reverse_sp (FFLXL)
#else
      DO K=2,KKP                                                                 
