...
contains
  ...
  real function gamma_kid(x, y)  
    ...
    if (.not. present(y))then
      ...
      if ((z+int(abs(z))).ne.0)then
        do while(z<3)
          ! Lets use a recursion relation for Gamma_Kid functions
      ...
      gamma_kid=f
       ...
       gamma_kid=max(0., gammaup(x,y))

    end if
    
  end function gamma_kid
  ...
  real(wp) function gammaup(a,x)
    ...
    gammaup=(gamma_kid(a))*(1-x**a*eigamma(a,x))
